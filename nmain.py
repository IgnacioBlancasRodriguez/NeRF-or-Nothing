...
def perform_NeRF(image):
    # Load the images
    training_dataset = torch.from_numpy(np.load('f_train1.pkl', allow_pickle=True))
    testing_dataset = torch.from_numpy(np.load('f_test1.pkl', allow_pickle=True))
    ...
    perform_training(model, model_optimizer, scheduler, data_loader, testing_dataset, n_epochs=16, device=device, tn=0, tf=1, n_bins=192, H=378,
          W=504)
