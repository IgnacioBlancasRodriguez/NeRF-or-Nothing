...
def generate_data(transforms_dir,start_idx,end_idx):
    ...
    for idx in range(start_idx,end_idx):
        ...
        # rays_o, rays_d = get_rays(height,width,focal_length,mat)
        rays_o,rays_d = get_ray_origins_and_directions(height,width,focal_length,mat)
        # rays_o, rays_d = rays_from_camera(False,400,400,focal_length,mat)
        ndc_o, ndc_d = ndc_rays(height,width,focal_length,near,rays_o,rays_d)
        # ndc_o, ndc_d = convert_to_ndc(rays_o,rays_d,focal_length,width,height,near)
        # ndc_o, ndc_d = rays_o, rays_d
        rgb = load_rgb_from_png(scaled_dir+scaled_prefix+str(idx)+scaled_file_type)

        # assert(len(ndc_o)==len(ndc_d) and len(ndc_o)==len(rgb))
        # for x in range(width):
        #     for y in range(height):
        #         row = [ndc_o[x][y][0],ndc_o[x][y][1],ndc_o[x][y][2],
        #                 ndc_d[x][y][0],ndc_d[x][y][1],ndc_d[x][y][2],
        #                 rgb[x][y][0]/255.0,rgb[x][y][1]/255.0,rgb[x][y][2]/255.0]
        #         data.append(row)
